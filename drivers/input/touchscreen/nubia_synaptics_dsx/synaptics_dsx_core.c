...
/* nubia custom key code not in upstream 4.9 input.h */
#ifndef KEY_PALM_LOCK
#define KEY_PALM_LOCK		0x2d3
#endif
...
//nubia for synaptics pinctrl
	...
	if (retval < 0) {
		dev_err(&pdev->dev,
				"%s: Failed to register msm_drm notifier client\n",
				__func__);
	}
#endif
		...
		unsigned long val, void *v)
{
	...
	if (!psy->desc || !psy->desc->get_property) {
	...
	if (psy->desc->get_property != NULL ) {
		retval = power_supply_get_property(psy,POWER_SUPPLY_PROP_STATUS,&prop);
	}
	if (retval != 0) {
