		...
		struct platform_device *pdev)
{
	...
	int panel_id_gpio, on_state, val, idx, count;
	enum of_gpio_flags flags;

	pr_debug("%s:%d: Select primary panel from dt\n",
					__func__, __LINE__);

	/* Panel auto-detect via qcom,panel-id-gpio when phandle list has >1 entry. */
	count = of_count_phandle_with_args(of_node,
					   "qcom,dsi-pref-prim-pan", NULL);
	idx = 0;
	if (count > 1) {
		panel_id_gpio = of_get_named_gpio_flags(of_node,
				"qcom,panel-id-gpio", 0, &flags);
		if (gpio_is_valid(panel_id_gpio)) {
			if (of_property_read_u32(of_node,
					"qcom,panel-id-on-state", &on_state))
				on_state = 1;
			if (!gpio_request(panel_id_gpio, "panel_id_gpio")) {
				gpio_direction_input(panel_id_gpio);
				val = gpio_get_value(panel_id_gpio);
				idx = (val == on_state) ? 0 : 1;
				if (idx >= count)
					idx = 0;
				pr_info("%s: panel-id gpio=%d val=%d on=%d -> idx=%d\n",
					__func__, panel_id_gpio, val,
					on_state, idx);
				gpio_free(panel_id_gpio);
			} else {
				pr_warn("%s: panel-id gpio %d request failed, default idx 0\n",
					__func__, panel_id_gpio);
			}
		}
	}

	dsi_pan_node = of_parse_phandle(of_node,
					"qcom,dsi-pref-prim-pan", idx);
	if (!dsi_pan_node) {
		pr_err("%s:can't find panel phandle at idx %d\n",
			__func__, idx);
		if (idx != 0)
			dsi_pan_node = of_parse_phandle(of_node,
