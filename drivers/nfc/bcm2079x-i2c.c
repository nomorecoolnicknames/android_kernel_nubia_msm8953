...
#define BCM2079X_RUNTIME_ADDRESS 0x38
...
static void bcm2079x_log_gpio_state(struct bcm2079x_dev_t *bcm2079x_dev,
				    const char *where)
{
	struct i2c_client *client = bcm2079x_dev->client;

	dev_info(&client->dev,
		 "%s: gpio readback en=%d wake=%d irq=%d addr=0x%04x flags=%04x\n",
		 where, gpio_get_value(bcm2079x_dev->en_gpio),
		 gpio_get_value(bcm2079x_dev->wake_gpio),
		 gpio_get_value(bcm2079x_dev->irq_gpio), client->addr,
		 client->flags);
}
...
static void set_client_addr(struct bcm2079x_dev_t *bcm2079x_dev, int addr)
{
	struct i2c_client *client = bcm2079x_dev->client;
	client->addr = addr;
	if (addr > 0x7F)
...
static void change_client_addr(struct bcm2079x_dev_t *bcm2079x_dev, int addr)
{
	struct i2c_client *client;
	int ret;
	int i;
	...
	cmd_size = sizeof(addr_data);
	payload_size = cmd_size - 1;
	addr_data[5] = addr & 0xFF;
	ret = 0;
	for (i = 1; i < payload_size; ++i)
		ret += addr_data[i];
	addr_data[payload_size] = (ret & 0xFF);
	dev_info(&client->dev,
		 "Change client device from (0x%04X) flag = "\
		 "%04x, addr_data[%d] = %d\n",
		 client->addr, client->flags, payload_size,
		 addr_data[payload_size]);
	write_size = cmd_size - offset;
	ret = i2c_master_send(client, addr_data + offset, write_size);
	if (ret != write_size) {
		client->addr = ALIAS_ADDRESS;
		client->flags &= ~I2C_CLIENT_TEN;
		dev_info(&client->dev,
			 ...
			 client->addr, client->flags, payload_size,
			 addr_data[payload_size]);
		ret = i2c_master_send(client, addr_data, cmd_size);
	}
...
static irqreturn_t bcm2079x_dev_irq_handler(int irq, void *dev_id)
{
	...
	__pm_wakeup_event(&bcm2079x_dev->wake_lock, 2000);
...
static unsigned int bcm2079x_dev_poll(struct file *filp, poll_table *wait)
{
	...
	if (bcm2079x_dev->count_irq > 0 && gpio_val) {
		...
		mask |= POLLIN | POLLRDNORM;
                dev_err(&bcm2079x_dev->client->dev, "797poll count_irq=0x%x,gpio_val:%d", bcm2079x_dev->count_irq,gpio_val);
	}
	spin_unlock_irqrestore(&bcm2079x_dev->irq_enabled_lock, flags);
				  ...
				  size_t count, loff_t *offset)
{
	...
	**/
	...
	mutex_unlock(&bcm2079x_dev->read_mutex);
        if ((ret == 3) &&(count == 3)&&(tmp[0]==0xff)&&(tmp[1] == 0xff)&&(tmp[2]==0xff)){
...
"r:%d c:%zu, 0x%02x, 0x%02x, 0x%02x",total, count, tmp[0], tmp[1], tmp[2]);
	return total;
}
				   ...
				   size_t count, loff_t *offset)
{
	...
	if (ret != count) {
		if ((bcm2079x_dev->client->flags & I2C_CLIENT_TEN) != I2C_CLIENT_TEN && bcm2079x_dev->error_write == 0) {
			dev_err(&bcm2079x_dev->client->dev,
				"write failed %d at addr 0x%04x flags %04x, trying address-change to 0x%02x\n",
				ret, bcm2079x_dev->client->addr,
				bcm2079x_dev->client->flags,
				BCM2079X_RUNTIME_ADDRESS);
			change_client_addr(bcm2079x_dev, BCM2079X_RUNTIME_ADDRESS);
			ret = i2c_master_send(bcm2079x_dev->client, tmp, count);
		}
		if (ret != count &&
		    bcm2079x_dev->error_write == 0) {
			dev_err(&bcm2079x_dev->client->dev,
				"write failed %d at addr 0x%04x flags %04x, retrying 10-bit address\n",
				ret, bcm2079x_dev->client->addr,
				bcm2079x_dev->client->flags);
...
static int bcm2079x_dev_open(struct inode *inode, struct file *filp)
{
	...
	bcm2079x_log_gpio_state(bcm2079x_dev, "open");

	return ret;
}

static void bcm2079x_power_ctl(struct bcm2079x_dev_t *bcm2079x_dev,
			       unsigned long arg)
{
	if (arg != 1)
		set_client_addr(bcm2079x_dev, bcm2079x_dev->original_address);
	...
	if (arg == 1) {
		/* Broadcom HAL treats asserted wake on POR as UPIO_OFF. */
	...
	bcm2079x_log_gpio_state(bcm2079x_dev, arg ? "power_on" : "power_off");
}
					 ...
					 unsigned int cmd, unsigned long arg)
{
	...
	case BCMNFC_CHANGE_ADDR:
		...
		break;
	case BCMNFC_SET_CLIENT_ADDR:
		dev_info(&bcm2079x_dev->client->dev,
			 "%s, BCMNFC_SET_CLIENT_ADDR (%x, %lx):\n", __func__, cmd,
			 arg);
		...
		case BCMNFC_POWER_CTL:
			dev_info(&bcm2079x_dev->client->dev,
				 ...
				 arg);
			bcm2079x_power_ctl(bcm2079x_dev, arg);
			break;
	case BCMNFC_WAKE_CTL:
		...
		//printk("zhanglongbo:arg is %lx\n",arg);
			...
			bcm2079x_log_gpio_state(bcm2079x_dev,
						arg ? "wake_on" : "wake_off");
...
static int nfc_pinctrl_init(struct device *dev)
{
	...
	if (IS_ERR_OR_NULL(bcm2079x_pctrl.nfc_gpio_state_suspend)) {
...
static int bcm2079x_parse_dt(struct device *dev, struct bcm2079x_platform_data * pdata)
{
	...
	/* PM8953 GPIO2 NFC clock request is configured by nfc_clk_default pinctrl. */
			   ...
			   const struct i2c_device_id *id)
{
	...
	ret = nfc_pinctrl_init(&client->dev);
	if (ret) {
		dev_err(&client->dev, "nfc pinctrl init failed: %d\n", ret);
	} else {
		ret = pinctrl_select_state(bcm2079x_pctrl.pinctrl,
				bcm2079x_pctrl.nfc_gpio_state_active);
		if (ret)
			dev_err(&client->dev,
				"cannot set pin to nfc_gpio_state_active: %d\n",
				ret);
		else
			dev_info(&client->dev,
				"selected nfc_gpio_state_active pinctrl state\n");
	}
	...
	bcm2079x_dev->client = client;
	    //bcm2079x_dev->tran_ver = 0;
	bcm2079x_log_gpio_state(bcm2079x_dev, "probe_gpio_init");
	...
	if (IS_ERR_OR_NULL(nfc_rf_clk)) {
		ret = IS_ERR(nfc_rf_clk) ? PTR_ERR(nfc_rf_clk) : -ENOENT;
		dev_err(&client->dev, "clk_get(ref_clk) failed: %d\n", ret);
		nfc_rf_clk = NULL;
	} else {
		ret = clk_prepare_enable(nfc_rf_clk);
		if (ret)
			dev_err(&client->dev,
				"clk_prepare_enable(ref_clk) failed: %d\n", ret);
		else
			dev_info(&client->dev,
				 "ref_clk prepared and enabled rate=%lu\n",
				 clk_get_rate(nfc_rf_clk));
	}
